...
class MetadataTransactionService {
     ...
     */
    createMetadataTransaction(deadline, networkType, metadataType, targetAddress, key, value, sourceAddress, targetId, maxFee = new UInt64_1.UInt64([0, 0])) {
        switch (metadataType) {
            ...
            case MetadataType_1.MetadataType.Mosaic:
                ...
                return this.createMosaicMetadataTransaction(deadline, networkType, targetAddress, targetId, key, value, sourceAddress, maxFee);
            case MetadataType_1.MetadataType.Namespace:
                ...
                return this.createNamespaceMetadataTransaction(deadline, networkType, targetAddress, targetId, key, value, sourceAddress, maxFee);
            default:
    ...
    createAccountMetadataTransaction(deadline, networkType, targetAddress, key, value, sourceAddress, maxFee) {
        return this.metadataRepository.getAccountMetadataByKeyAndSender(targetAddress, key.toHex(), sourceAddress).pipe(operators_1.map((metadata) => {
            ...
            if (error && error.statusCode && error.statusCode === 404) {
                const newValueBytes = Convert_1.Convert.utf8ToUint8(value);
                return rxjs_1.of(AccountMetadataTransaction_1.AccountMetadataTransaction.create(deadline, targetAddress, key, newValueBytes.length, value, networkType, maxFee));
            }
    ...
    createMosaicMetadataTransaction(deadline, networkType, targetAddress, mosaicId, key, value, sourceAddress, maxFee) {
        return this.metadataRepository.getMosaicMetadataByKeyAndSender(mosaicId, key.toHex(), sourceAddress).pipe(operators_1.map((metadata) => {
            const currentValueByte = Convert_1.Convert.utf8ToUint8(metadata.metadataEntry.value);
            const newValueBytes = Convert_1.Convert.utf8ToUint8(value);
            ...
            if (error && error.statusCode && error.statusCode === 404) {
                const newValueBytes = Convert_1.Convert.utf8ToUint8(value);
                return rxjs_1.of(MosaicMetadataTransaction_1.MosaicMetadataTransaction.create(deadline, targetAddress, key, mosaicId, newValueBytes.length, value, networkType, maxFee));
            }
            throw Error(err.message);
        }));
    }
    ...
    createNamespaceMetadataTransaction(deadline, networkType, targetAddress, namespaceId, key, value, sourceAddress, maxFee) {
        return this.metadataRepository.getNamespaceMetadataByKeyAndSender(namespaceId, key.toHex(), sourceAddress).pipe(operators_1.map((metadata) => {
            const currentValueByte = Convert_1.Convert.utf8ToUint8(metadata.metadataEntry.value);
            const newValueBytes = Convert_1.Convert.utf8ToUint8(value);
            ...
            if (error && error.statusCode && error.statusCode === 404) {
                const newValueBytes = Convert_1.Convert.utf8ToUint8(value);
                return rxjs_1.of(NamespaceMetadataTransaction_1.NamespaceMetadataTransaction.create(deadline, targetAddress, key, namespaceId, newValueBytes.length, value, networkType, maxFee));
            }
            throw Error(err.message);
        }));
    }
