...
var symbolpay;
var vagabondenergyget;


function valuedisplay(){

	document.getElementById('displaybuy1').value = sessionStorage.getItem("energyamountfixed");
	document.getElementById('displaybuy2').value = sessionStorage.getItem("buyingpricefixed");
	document.getElementById('displaybuy3').value = sessionStorage.getItem("totalcostfixed");
	vagabondenergyget = document.getElementById('displaybuy1').value;
	symbolpay = document.getElementById('displaybuy3').value;

}

function escrowtransactionbuyenergy(){
	sendenergy();
	sendsymbol();
}
...
function sendsymbol(){
	...
	const transferTransaction = symbol_sdk_1.TransferTransaction.create(symbol_sdk_1.Deadline.create(), recipientAddress, [new symbol_sdk_1.Mosaic(networkCurrencyMosaicId, symbol_sdk_1.UInt64.fromUint(vagabondenergyget * Math.pow(10, networkCurrencyDivisibility)))], symbol_sdk_1.PlainMessage.create('Fast Forward'), networkType, symbol_sdk_1.UInt64.fromUint(2000000));
	/* end block 01 */
	/* start block 02 */
	// replace with sender private key
